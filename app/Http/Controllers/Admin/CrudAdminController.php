...
class CrudAdminController extends AppBaseController
{
    ...
    public function index()
    {
        ...
        {
            $this->data = array_add($this->data,'url_export',route($this->routePrefix.'.export',['_TYPE_']));
        }      
        if (\Route::has($this->routePrefix.'.change-enabled'))
        {
            $this->data = array_add($this->data,'url_change_enabled',route($this->routePrefix.'.change-enabled'));
        }                	
    }
